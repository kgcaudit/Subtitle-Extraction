...
정확도는 **실제 한글 DVD 자막 영화 두 편**으로 재었습니다. 자막을 화면에 그려 본
그림을 눈으로 읽어 정답지를 손으로 만들고, **휴대폰과 PC 에서 각각** 끝까지 돌려
따로 맞춰 봤습니다.

| | 영화 A (1,577줄) | 영화 B (1,661줄) |
|---|---|---|
| 정답지 | 손으로 받아 적은 64줄 | 손으로 받아 적은 60줄 |
| 전처리를 손보기 전 | 91.53% | — |
| **지금 — 폰** | **99.55%** (60/64) | **98.62%** (53/60) |
| **지금 — PC** | **99.44%** (59/64) | **98.35%** (53/60) |
...
실제 영화 자막 두 편으로 양쪽을 맞춰 보면 **자막 수와 시각이 완전히 같고**
(1,577줄·1,661줄 모두 밀리초까지), 그림 해독은 **픽셀 하나까지 같으며**(605만 픽셀
대조, 차이 0), 글자는 **99.91% / 99.87%** 일치합니다. 남은 차이는 인식 엔진이
서로 달라(WebAssembly 대 네이티브) 생기는 것입니다.
