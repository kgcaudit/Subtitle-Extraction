...
파일을 고르면 **고르는 칸에 그 파일 이름과 크기가 남습니다** — 화면만 봐도 어느
영상에서 뽑은 자막인지 알 수 있습니다.

뽑은 자막의 미리보기에는 **자막마다 시작 시각**이 앞에 붙습니다
(`00:01:02,300`) — 영상의 어느 대목인지, 어디까지가 자막 하나인지 보입니다.

추출 단추와 받기 단추는 **화면을 따라다닙니다.** 자막 28개짜리 파일에서는 목록
맨 끝까지(작은 폰에서 4화면) 내려가야 단추에 닿았습니다. 돌아가는 동안에는 그
막대에 진행이 그려집니다.
...
52개). 기준 자료는 `python3 tests/make_web_fixtures.py` 와
